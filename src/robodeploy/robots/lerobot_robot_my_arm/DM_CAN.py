...
class Motor:
    ...
    def recv_data(self, state: int, q: float, dq: float, tau: float):
        self.state = state
        self.state_q = q
        self.state_dq = dq
        self.state_tau = tau
...
class MotorControl:
    ...
    Limit_Param = [[12.5, 30, 10], [12.5, 50, 10], [12.5, 8, 28], [12.5, 10, 28],
                   ...
                   [12.5 , 280 , 1],[12.5 , 45 , 10],[12.5 , 45 , 10], [12.5, 20, 120]]
    ...
    def control_pos_force(self, Motor, Pos_des: float, Vel_des, i_des):
        ...
        Vel_uint = np.uint16(Vel_des * 100)
        ides_uint = np.uint16(i_des * 10000)
        ...
        if CMD == 0x11:
            if CANID != 0x00:
                if CANID in self.motors_map:
                    state = np.uint16(np.uint16(data[0]) >> 4)
                    ...
                    self.motors_map[CANID].recv_data(state, recv_q, recv_dq, recv_tau)
            else:
                ...
                if MasterID in self.motors_map:
                    ...
                    recv_dq = uint_to_float(dq_uint, -DQ_MAX, DQ_MAX, 12)
                    recv_tau = uint_to_float(tau_uint, -TAU_MAX, TAU_MAX, 12)
                    self.motors_map[MasterID].recv_data(state, recv_q, recv_dq, recv_tau)

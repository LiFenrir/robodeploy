...
# Copyright 2024 The robodeploy contributors. All rights reserved.
#
...
```shell
python -m robodeploy.find_cameras
```
"""

# NOTE(Steven): RealSense can also be identified/opened as OpenCV cameras. If you know the camera is a RealSense, use the `find-cameras realsense` flag to avoid confusion.
...
from robodeploy.cameras.configs import ColorMode
from robodeploy.cameras.opencv import OpenCVCamera, OpenCVCameraConfig
...
def find_all_realsense_cameras() -> list[dict[str, Any]]:
    ...
    try:
        from robodeploy.cameras.realsense import RealSenseCamera

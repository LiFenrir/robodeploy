...
# RTC（Real-Time Chunking — 服务端 RTC 引导 + 客户端 overlap blending）
#------------------------------------------------------------------------------
USE_RTC=true                                 # true 启用 RTC | false 回退 StreamBuffer
RTC_EXECUTION_HORIZON=11                     # smooth_window = RTC 引导约束窗口 + 客户端 blend overlap
RTC_QUEUE_SIZE=25                            # 固定队列容量 = 每次推理返回的 action 步数
RTC_INFERENCE_STEP_INTERVAL=7                # 步数驱动触发: 每消费 N 步发起一次推理 (= 推理耗时折合步数)
...
if [ "$CONTROL_MODE" != "teleop" ]; then
    if [ "$USE_RTC" = true ]; then
        ...
        RTC_ARGS+=(--rtc_queue_size "$RTC_QUEUE_SIZE")
        RTC_ARGS+=(--rtc_inference_step_interval "$RTC_INFERENCE_STEP_INTERVAL")
    else
...
print_config() {
    ...
    if [ "$CONTROL_MODE" != "teleop" ]; then
        ...
        if [ "$USE_RTC" = true ]; then
            ...
            echo "  Interval:   ${RTC_INFERENCE_STEP_INTERVAL} 步   Warmup: ${WARMUP_ROUNDS}"
        fi
...
if [ "$CONTROL_MODE" != "teleop" ]; then
    ...
    if [ "$USE_RTC" = true ]; then
        RTC_ARGS=(--use_rtc true --rtc_execution_horizon "$RTC_EXECUTION_HORIZON" --rtc_queue_size "$RTC_QUEUE_SIZE" --rtc_inference_step_interval "$RTC_INFERENCE_STEP_INTERVAL")
    else
        RTC_ARGS=(--use_rtc false)
    fi
